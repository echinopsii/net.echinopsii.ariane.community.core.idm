...
public class IDMJPAProviderImpl implements IDMJPAProvider {
    ...
    private static final String IDM_TXPERSISTENCE_CONSUMER_SERVICE_NAME = "IDM JPA Persistence Provider";
    ...
    public void validate() throws InterruptedException {
        log.info("{} is starting !", new Object[]{IDM_TXPERSISTENCE_CONSUMER_SERVICE_NAME});
        while (persistenceProvider==null && !isConfigurationValid) {
            log.warn("Persistence provide or valid config is missing for {}. Sleep some times...", IDM_TXPERSISTENCE_PERSISTENCE_UNIT_NAME);
            Thread.sleep(10);
        }
        this.start();
        isStarted = true;
    ...
    public void invalidate() {
        log.info("{} is stopping !", new Object[]{IDM_TXPERSISTENCE_CONSUMER_SERVICE_NAME});
        this.stop();
        isStarted = false;
    ...
    public synchronized void updated(final Dictionary properties) {
        log.info("{} is being updated by {}", new Object[]{IDM_TXPERSISTENCE_CONSUMER_SERVICE_NAME, Thread.currentThread().toString()});
        ...
        if (isStarted) {
            final Runnable applyConfigUpdate = new Runnable() {
                @Override
                public void run() {
                    log.info("{} will be restart to apply configuration changes...", IDM_TXPERSISTENCE_CONSUMER_SERVICE_NAME);
                    stop();
                    start();
                }
            };
